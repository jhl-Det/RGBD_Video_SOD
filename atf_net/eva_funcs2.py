...
def confusion_matrix(pred, true, num_class=2):
    ''' 返回混淆矩阵 '''
    pred = pred.flatten()
    true = true.flatten()
    assert ((0 <= pred) & (pred < num_class)).all()
    mask = (0 <= true) & (true < num_class)
    label = num_class * true[mask] + pred[mask]
    matrix = np.bincount(label, minlength=num_class**2)
    matrix = matrix.reshape(num_class, num_class)
    return matrix
...
def f_measure(confusion_matrix):
    ...
    rec = recall(confusion_matrix)
    b = 0.3
    return (1+b) * pre * rec / (b * pre + rec + 1e-20)


def fun_eval_e(y_pred, y, num, cuda=False):
...
def fun_eval_pr(y_pred, y, num, cuda=False):
    ...
    for i in range(num):
        y_temp = (y_pred >= thlist[i]).astype(np.float32)
        tp = (y_temp * y).sum()
...
## metric funcs
###############################################################################
def eval_mae(pred,gt,cuda=False):
...
def eval_Smeasure(pred,gt,cuda=False):
...
def eval_fmeasure(pred, gt, cuda=False):
    ...
    with torch.no_grad():
        ...
        prec, recall = fun_eval_pr(pred, gt, 2)
    
    score = (1 + beta2) * prec * recall / (beta2 * prec + recall + 1e-20)
