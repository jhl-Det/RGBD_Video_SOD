...
class SalObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root,depth_root, trainsize):
        ...
        videos_list = sorted(os.listdir(image_root))
        for video in videos_list:
            if video == '.DS_Store':
                continue
            rgb_path = os.path.join(image_root, video) + '/rgb'
            rgb_list = [x for x in sorted(os.listdir(rgb_path)) if x.endswith('.png')]
            
            for x in rgb_list:
                if x.endswith('.png'):
                    self.images.append(os.path.join(rgb_path, x)) 
...
class test_dataset:
    ...
    def load_data(self):
        ...
        dirname = self.images[self.index].split('/')[-3]
        name = dirname + '/' + name
